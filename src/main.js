...
function updateHud() {
  el.turns.textContent = String(state.turn);
...
function setDemoLabel() {
  // innerHTML, not textContent: the key hint is part of the label and would
  // otherwise be wiped the first time the button is drawn.
  if (!demo) { el.demo.innerHTML = 'DEMO <kbd>P</kbd>'; el.demo.classList.remove('running'); return; }
...
const KEYS = {
  ...
  Space: 'strike', KeyE: 'strike',
};

window.addEventListener('keydown', (e) => {
  if (e.metaKey || e.ctrlKey) return;
  const dir = KEYS[e.code];
  // preventDefault first: Space would otherwise activate whichever button
  // happens to hold focus, and scroll the page on the way.
  ...
  else if (e.code === 'KeyP') { e.preventDefault(); toggleDemo(); }
...
// Touch: swipe to move, tap to strike. A tap is not a way to pass the turn —
// striking is only legal with a sentinel beside you.
let touch = null;
...
canvas.addEventListener('touchend', (e) => {
  ...
  touch = null;
  if (Math.abs(dx) < 24 && Math.abs(dy) < 24) { doMove('strike'); return; }
